import boto3
import os
from us_visa.constants import AWS_SECRET_ACCESS_ENV_KEY, AWS_ACCESS_KEY_ID_ENV_KEY,REGION_NAME
...
class S3Client:
    ...
    def __init__(self, region_name=REGION_NAME):
        ...
        if S3Client.s3_resource==None or S3Client.s3_client==None:
            ...
            __secret_access_key = os.getenv(AWS_SECRET_ACCESS_ENV_KEY, )
            if __access_key_id is None:
                raise Exception(f"Environment variable: {AWS_ACCESS_KEY_ID_ENV_KEY} is not not set.")
            if __secret_access_key is None:
                raise Exception(f"Environment variable: {AWS_SECRET_ACCESS_ENV_KEY} is not set.")
